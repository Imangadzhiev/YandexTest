...
public class fav_fr extends Fragment {
    ...
    public View onCreateView(final LayoutInflater inflater, ViewGroup container, Bundle savedInstanceState) {
        ...
        etSearch.setHint(getString(R.string.search_in_favorites));
    ...
    public static void refresh(){
        ...
        if (arrayList.size() > 0){
            adapter = new favAdapter(ctx, arrayList, "fav");
            LinearLayoutManager llm = new LinearLayoutManager(ctx);
            rv.setAdapter(adapter);
            rv.setHasFixedSize(true);
            rv.setLayoutManager(llm);
        } else {
            arrayList.add(new itemHistory(ctx.getString(R.string.no_favorites),"","",new Date()));
    ...
    public boolean onOptionsItemSelected(MenuItem item) {
        switch (item.getItemId()) {
            case R.id.action_clear:
                editor.putString(SP.FAVORITES, null).apply();
                refresh();
                break;
            default:
                break;
